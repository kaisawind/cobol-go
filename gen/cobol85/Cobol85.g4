 ...
 * executes COPY and REPLACE statements.
 */
...
identificationDivision: (IDENTIFICATION | ID) DIVISION DOT_FS programIdParagraph
...
collatingSequenceClause:
	PROGRAM? COLLATING? SEQUENCE (IS? alphabetName+) collatingSequenceClauseAlphanumeric?
...
symbolicCharactersClause:
	SYMBOLIC CHARACTERS? (FOR? (ALPHANUMERIC | NATIONAL))? symbolicCharacters+ (
		IN alphabetName
	)?;
...
organizationClause: (ORGANIZATION IS?)? (
		LINE
		| RECORD BINARY
		| RECORD
		| BINARY
...
ioControlParagraph:
	I_O_CONTROL DOT_FS (fileName DOT_FS)? (
...
fileSection: FILE SECTION DOT_FS fileDescriptionEntry*;

fileDescriptionEntry: (FD | SD) fileName (
...
blockContainsClause:
	BLOCK CONTAINS? integerLiteral blockContainsTo? (
		RECORDS
		| CHARACTERS
	)?;
...
dataBaseSection: DATA_BASE SECTION DOT_FS dataBaseSectionEntry*;
...
destinationTableClause:
	DESTINATION TABLE OCCURS integerLiteral TIMES (
...
screenSection: SCREEN SECTION DOT_FS screenDescriptionEntry*;
...
screenDescriptionColumnClause: (COLUMN | COL) (
		NUMBER? IS? (PLUS | PLUSCHAR | MINUSCHAR)
...
screenDescriptionSignClause: (SIGN IS?)? (LEADING | TRAILING) (
		SEPARATE CHARACTER?
	)?;
...
reportSection: REPORT SECTION DOT_FS reportDescription*;
...
reportGroupDescriptionEntryFormat1:
	integerLiteral dataName reportGroupLineNumberClause? reportGroupNextGroupClause?
		reportGroupTypeClause reportGroupUsageClause? DOT_FS;
...
reportGroupTypeControlHeading: (CONTROL HEADING | CH) (
		FINAL
		| dataName
	);
...
reportGroupTypeControlFooting: (CONTROL FOOTING | CF) (
		FINAL
		| dataName
	);
...
libraryEntryProcedureClauseFormat2:
	ENTRY_PROCEDURE programName libraryEntryProcedureForClause? libraryEntryProcedureWithClause?
		libraryEntryProcedureUsingClause? libraryEntryProcedureGivingClause?;
...
dataDescriptionEntryFormat1: (INTEGERLITERAL | LEVEL_NUMBER_77) (
		FILLER
		| dataName
	)? (
		dataRedefinesClause
...
dataDescriptionEntryExecSql: EXECSQLLINE+ DOT_FS?;
...
dataSignClause: (SIGN IS?)? (LEADING | TRAILING) (
		SEPARATE CHARACTER?
	)?;
...
procedureDivisionGivingClause: (GIVING | RETURNING) dataName;
...
procedureDivisionByReference: (OPTIONAL? (identifier | fileName))
	| ANY;
...
callStatement:
	CALL (identifier | literal) callUsingPhrase? callGivingPhrase? onOverflowPhrase?
		onExceptionClause? notOnExceptionClause? END_CALL?;
...
callByContent: (ADDRESS OF | LENGTH OF?)? identifier
	| literal
	| OMITTED;
...
closeReelUnitStatement: (REEL | UNIT) (FOR? REMOVAL)? (
		WITH? (NO REWIND | LOCK)
	)?;

closeRelativeStatement: WITH? (NO REWIND | LOCK);

closePortFileIOStatement: (WITH? NO WAIT | WITH WAIT) (
		USING closePortFileIOUsing+
	)?;
...
disableStatement:
	DISABLE (INPUT TERMINAL? | I_O TERMINAL | OUTPUT) cdName WITH? KEY (
		identifier
		| literal
	);
...
enableStatement:
	ENABLE (INPUT TERMINAL? | I_O TERMINAL | OUTPUT) cdName WITH? KEY (
		literal
		| identifier
	);
...
execCicsStatement: EXECCICSLINE+;
...
execSqlImsStatement: EXECSQLIMSLINE+;
...
inspectBeforeAfter: (BEFORE | AFTER) INITIAL? (
		identifier
		| literal
	);
...
readStatement:
	READ fileName NEXT? RECORD? readInto? readWith? readKey? invalidKeyPhrase? notInvalidKeyPhrase?
...
receiveStatement:
	RECEIVE (receiveFromStatement | receiveIntoStatement) onExceptionClause? notOnExceptionClause?
		END_RECEIVE?;
...
sendStatementSync: (identifier | literal) sendFromPhrase? sendWithPhrase? sendReplacingPhrase?
		sendAdvancingPhrase?;
...
sortStatement:
	SORT fileName sortOnKeyClause+ sortDuplicatesPhrase? sortCollatingSequencePhrase?
		sortInputProcedurePhrase? sortUsing* sortOutputProcedurePhrase? sortGivingPhrase*;
...
stringStatement:
	STRING stringSendingPhrase+ stringIntoPhrase stringWithPointerPhrase? onOverflowPhrase?
...
subtractCorrespondingStatement: (CORRESPONDING | CORR) qualifiedDataName FROM
...
useStatement: USE (useAfterClause | useDebugClause);

useAfterClause:
	GLOBAL? AFTER STANDARD? (EXCEPTION | ERROR) PROCEDURE ON? useAfterOn;

useAfterOn: INPUT | OUTPUT | I_O | EXTEND | fileName+;

useDebugClause: FOR? DEBUGGING ON? useDebugOn+;
...
conditionNameReference:
	conditionName (
		inData* inFile? conditionNameSubscriptReference*
		| inMnemonic*
	);
...
qualifiedDataNameFormat1: (dataName | conditionName) (
...
fragment HEXNUMBER: X '"' [0-9A-F]+ '"' | X '\'' [0-9A-F]+ '\'';
